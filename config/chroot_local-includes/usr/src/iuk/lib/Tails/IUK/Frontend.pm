...
method refresh_signing_key () {
    ...
    $error_msg =
        __(
            q{<b>SITREP: signing key refresh failed.</b>\n\n}.
            q{<b>Upgrade intel cannot be verified from command.</b>\n\n}.
            q{Check network link and perform a tactical reset to try again.\n\n}.
    ...
    try {
        ...
        $self->fatal(
            $error_msg,
            title => __(q{SITREP: signing key download failure}),
    ...
    $success or $self->fatal(
        $error_msg,
        title => __(q{SITREP: signing key update failure}),
        debugging_info => $self->encoding->decode(errf(
            "exit code: %{exit_code}i\n\n".
            "stdout:\n%{stdout}s\n\n".
            "stderr:\n%{stderr}s",
...
method get_upgrade_description () {
    ...
    my ($stdout, $stderr, $success, $exit_code) = $self->fatal_run_cmd(
        ...
        error_title => __(q{SITREP: upgrade scan failure}),
        error_msg   => __(
            "<b>Unable to confirm upgrade availability from command.</b>\n\n".
            "Check network link and perform a tactical reset to try again.\n\n".
...
method run () {
    ...
    try {
        ...
        unless ($upgrade_description->contains_upgrade_path) {
            $self->info(__("Delta-1, you are green across the board. Proceed to OBJ."));
            exit(0);
        }

        $self->info(__(
                'SITREP: this DELTA build is outdated and may expose operational risk.'
        ));
        ...
        $self->fatal(
            __(
                "SITREP: an error occurred while scanning for upgrades.\n".
                "This should not happen. Please report a bug.",
            ),
            title => __(q{SITREP: upgrade detection failure}),
            debugging_info => $self->encoding->decode($_),
        );
    };
    ...
    if (! defined($upgrade_path)) {
        if ($upgrade_description->contains_full_upgrade_path) {
            ...
            $self->fatal(
                __(
                    "An incremental redeploy path exists, but no full redeploy path is available.\n".
                    "This should not happen. Please report a bug."
                ),
                title => __(
                    q{SITREP: upgrade detection failure}
                ),
            );
        }
    }
    ...
    if ($upgrade_type eq 'incremental') {
        exit(0) unless($self->dialog(
            __x(
                "<b>Mission update available: redeploy to {name} {version}.</b>\n\n".
                "For more information about this new version, go to {details_url}\n\n".
                "Close non-essential applications before redeploy.\n".
                "Package sync can take from minutes to hours depending on the link.\n\n".
                 "Package size: {size}\n\n".
                 "Execute redeploy now?",
            ...
            title        => __(q{Mission update available}),
            ok_label     => __(q{Redeploy now}),
            cancel_label => __(q{Redeploy later}),
            ));
        ...
        exit(0) unless($self->dialog(
            __x(
                "<b>Manual redeploy required for {name} {version}.</b>\n\n".
                "For more information about this new version, go to {details_url}\n\n".
                "Automatic redeploy is not possible ".
                "your device to this new version: {explanation}.\n\n".
                "For manual redeploy procedure, go to {manual_upgrade_url}",
                details_url        => $upgrade_path->{'details-url'},
                name               => $upgrade_description->product_name,
                version            => $upgrade_path->{version},
            ...
            title => __(q{New mission build available}),
            type  => 'info',
        ));
