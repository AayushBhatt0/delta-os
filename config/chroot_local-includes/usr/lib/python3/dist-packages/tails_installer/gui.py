    ...
    def run(self):
        ...
        try:
            ...
            if not self.live.drive['uuid'] and not self.live.label:
                self.status(_('SITREP: Cannot tag target or obtain '
                              'the UUID of your device. Mission cannot continue.'))
            ...
            self.status(_('Deployment complete. Delta-1 green across the board. (%s)') % duration)
            self.installation_complete()

        except Exception as ex:
            self.status(ex)
            self.status(_('SITREP: Deployment failed. COMMS BLACKOUT on write path.'))
...
class TailsInstallerWindow(Gtk.ApplicationWindow):
    ...
    def _build_ui(self):
        ...
        self.set_title(_('DELTA Deployment Console'))
    ...
    def on_radio_button_source_iso_toggled(self, radio_button):
        ...
        if active_radio.get_label() == _('Clone the current DELTA image'):
            ...
            self.__filechooserbutton_source_file.set_sensitive(False)
        elif active_radio.get_label() == _('Use a downloaded DELTA ISO image'):
    ...
    def on_target_changed(self, combobox_target):
        ...
        if self.live.device_can_be_upgraded(device):
            ...
            self.force_reinstall = False
            self.__button_start.set_label(_('Redeploy'))
            self.__help_link.set_label(_('Manual Redeploy Instructions'))
            ...
            self.opts.partition = True
            self.force_reinstall = True
            self.__button_start.set_label(_('Deploy'))
            self.force_reinstall_button_available = False
            self.__button_force_reinstall.set_visible(False)
            self.__help_link.set_label(_('Deployment Instructions'))
    ...
    def warn_ISO_not_selected(self):
        self.show_confirmation_dialog(_('No ISO package selected'),
                          _('Select a DELTA ISO package.'),
                                      True)
    ...
    def populate_devices(self, *args, **kw):
        ...
        def add_devices():
            ...
            if not len(self.live.drives):
                ...
                self.__label_infobar_title.set_text(
                        _('No device suitable for DELTA deployment was found'))
                self.__label_infobar_details.set_text(
                    _('Insert a USB or SD target of at least %0.1f GB to continue mission prep.')
            ...
            for device, info in list(self.live.drives.items()):
                ...
                if not info['removable']:
                    message = _('The USB stick "%(pretty_name)s"'
                                ...
                                ' manufacturer and DELTA will fail to boot from it.'
                                ' Try another target model.') % {
                ...
                if not info['is_device_big_enough_for_installation']:
                    message = _('The device "%(pretty_name)s"'
                                ' is too small for DELTA deployment'
                                ' (at least %(size)s GB is required).') % {
                                'pretty_name': pretty_name,
                                'size': (float(
                ...
                if self.opts.clone \
                   and not info['is_device_big_enough_for_upgrade']:
                    message = _('To redeploy device "%(pretty_name)s"'
                                ' from this DELTA image, you need'
                                ' a downloaded DELTA ISO image:\n'
                                '%(dl_url)s') % {
                                    'pretty_name': pretty_name,
        ...
        try:
            ...
            self.__label_infobar_title.set_text(
                    _('SITREP: an error occurred while deploying DELTA'))
    ...
    def on_installation_complete(self, data=None):
        ...
        dialog = Gtk.MessageDialog(parent=self,
                                   ...
                                   message_format=_('Deployment complete. Delta-1 green across the board.'))
        dialog.run()
        self.close()

    def show_confirmation_dialog(self, title, message, warning,
                                 label_string=_('Deploy')):
        if warning:
    ...
    def begin(self):
        ...
        if not self.opts.partition:
            try:
                ...
                self.status(_('SITREP: Unable to mount target device'))
                self.enable_widgets(True)
                return
        ...
        if self.opts.partition:
            if not self.confirmed:
                if self.show_confirmation_dialog(
                        _('Confirm target deployment device'),
                                                _('%(size)s %(vendor)s %(model)s device (%(device)s)\n\n'
                                                    'Warning: this action will neutralize all data on target media.') %
            ...
            if self.devices_with_persistence:
                persistence_message = _('\n\nPersistent storage on this target will be preserved.')
            msg = _('Target asset: %(description)s%(persistence_message)s') % {
                'description': description,
                'persistence_message': persistence_message,
            }
                if self.show_confirmation_dialog(_('Confirm target deployment device'),
                    msg, False, label_string=_('Redeploy')):
    ...
    def select_source_iso(self, isofile):
        if not os.access(isofile, os.R_OK):
            self.status(_('SITREP: Selected package is unreadable. '
                          'Fix permissions or select another package.'))
            return False
        try:
            self.live.source = LocalIsoSource(path=isofile)
        except Exception as ex:
            self.status(_('SITREP: Unable to use selected package. '
                          'You may have better luck if you move your ISO '
                          'to the root of your drive (ie: C:\)'))
            self.live.log.exception(ex.args[0])
            return False

        self.live.log.info(_('Package %(filename)s loaded into staging')
